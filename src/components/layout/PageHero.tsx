...
export function PageHero({
  ...
  return (
    <section className="relative z-0 bg-surface-dark text-white overflow-hidden min-h-[280px] scroll-mt-16 lg:scroll-mt-[7.125rem]">
      {backgroundImage ? (
        <>
          <Image
            src={backgroundImage}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-16 lg:pt-8 lg:pb-20">
        {breadcrumb && breadcrumb.length > 0 && (
          <nav
            aria-label="Breadcrumb"
            className="flex flex-wrap items-center gap-x-1 gap-y-1 text-sm text-slate-300 mb-5 sm:mb-6"
          >
            ...
            {breadcrumb.map((crumb, i) => (
              <span key={`${crumb.label}-${i}`} className="flex items-center gap-1 min-w-0">
                ...
                {crumb.href && i < breadcrumb.length - 1 ? (
                  ...
                  <span className="text-slate-100 truncate">{crumb.label}</span>
                )}
              </span>
            ))}
          </nav>
