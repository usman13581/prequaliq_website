...
export function ProjectModal({ open, onClose }: ProjectModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} ariaLabelledBy="project-modal-title" sheetOnMobile>
      <div
        className="relative w-full max-w-xl max-h-[min(92dvh,900px)] sm:max-h-[min(90vh,900px)] bg-card rounded-t-2xl sm:rounded-2xl border border-border shadow-2xl flex flex-col overflow-hidden"
        ...
        <div className="flex items-start justify-between gap-4 px-5 sm:px-6 py-4 sm:py-5 border-b border-border bg-surface flex-shrink-0">
          <div className="min-w-0">
            <h2 id="project-modal-title" className="text-lg sm:text-xl font-bold text-foreground">
              {t.project.modal.title}
            </h2>
            <p className="text-sm text-muted mt-1">{t.project.modal.subtitle}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
